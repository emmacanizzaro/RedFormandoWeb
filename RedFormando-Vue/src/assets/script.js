...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function setupHamburgerMenus() {
    ...
    hamburgerBtns.forEach((hamburgerBtn) => {
      ...
      const menuHamburguesa = hamburgerBtn.parentElement.querySelector('.menu-hamburguesa')
      ...
      hamburgerBtn.addEventListener('click', (e) => {
        ...
        document.querySelectorAll('.menu-hamburguesa.open').forEach((otherMenu) => {
          if (otherMenu !== menuHamburguesa) {
            otherMenu.classList.remove('open')
            const otherBtn = otherMenu.parentElement.querySelector('.js-hamburger-menu')
            if (otherBtn) {
...
// === INSCRIPCIÓN EVENTO ===
document.addEventListener('DOMContentLoaded', function () {
  const abrirInscripcionBtn = document.getElementById('abrir-inscripcion-evento')
  ...
  window.addEventListener('keydown', function (event) {
    if (inscripcionSection && event.key === 'Escape' && !inscripcionSection.hidden) {
      inscripcionSection.hidden = true
      inscripcionSection.setAttribute('aria-hidden', 'true')
      inscripcionSection.classList.remove('is-open')
      document.body.classList.remove('modal-open')
...
const checkoutCurrencyInputs = document.querySelectorAll('input[name="checkout-currency"]')
...
function getSelectedCurrency() {
  const selected = Array.from(checkoutCurrencyInputs).find((input) => input.checked)
...
function syncFamilyPhotoBottomAlignment() {
  ...
  const targetHeight = Math.floor(leftHeight - workersTableHeight - gap - visualOffset)
...
function setupCheckoutForm() {
  ...
  loadCheckoutData()
  ;[checkoutName, checkoutPhone, checkoutEmail, checkoutNotes].forEach((field) => {
    if (!field) return
